...
describe("MainCtrl", async () => {
    ...
    it('should return false because verified before', async () => {
        const payment = await MainCtrl.verifyPaymentInfo(
            '2MwUckEwJxfezMT8prUfNYX9x5uVd1sEaXj',
            '2ac7c35a3259a418f907089face8d40a1f4440860549e7d6135a0629744c9451'
        );
        console.log(payment);
        assert(!payment);
    });
    ...
    it('should add a payment info to db', async () => {
        const addedPayment = await dbCtrl.addPaymentTx(txHash, 1,100, new Date(Date.now()));
        console.log(addedPayment)
        assert(addedPayment);
    });

    it('should return true because payment is already in db', async () => {
        const addedPayment = await dbCtrl.getPayment(txHash, 1);
        console.log(addedPayment)
        assert(addedPayment);
    });

    it('should return false because payment is not yet in the db (wrong vout)', async () => {
        const addedPayment = await dbCtrl.getPayment(txHash, 2);
        console.log(addedPayment)
        assert(!addedPayment);
    });

    it('should return false because payment is not yet in the db', async () => {
        const addedPayment = await dbCtrl.getPayment("cafe", 1);
        console.log(addedPayment)
        assert(!addedPayment);
    });
});
